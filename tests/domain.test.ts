...
describe('availability and privacy',()=>{
 ...
 it('makes every slot unavailable for a whole-day block',()=>{const slots=computeSlots({open:'10:00',close:'19:00',days:[0,1,2,3,4,5,6]},'2030-01-01',60,15,[{start_minute:0,end_minute:1440}],0);expect(slots.length).toBeGreaterThan(0);expect(slots.every(slot=>!slot.available)).toBe(true);});
...
describe('transactional booking invariants',()=>{
 ...
 it('rejects a whole-day block when a confirmed appointment exists',()=>{insert(db,'all-day-appointment');db.exec("UPDATE bookings SET status='approved' WHERE id='all-day-appointment'");expect(()=>db.exec("INSERT INTO blocks VALUES('all-day','studio-mei','2030-01-01',0,1440)")).toThrow('APPOINTMENT_EXISTS');});
