...
test('sign out requires confirmation and stays on Account',async({page})=>{
 ...
 expect((await (await page.request.get('/api/session')).json()).user).not.toBeNull();
 await page.getByRole('button',{name:'Sign out',exact:true}).click();
 const response=page.waitForResponse(r=>r.url().endsWith('/api/logout'));
 await dialog.getByRole('button',{name:'Sign out',exact:true}).click();
 expect((await response).ok()).toBe(true);
 await expect.poll(async()=>((await (await page.request.get('/api/session')).json()).user)).toBeNull();
 await expect(page).toHaveURL(/\/account$/);
 await expect(page.getByRole('navigation',{name:'Main navigation'}).last().getByRole('link',{name:'Account'})).toHaveAttribute('aria-current','page');
