...
function tabMatches(pathname:string,search:string,to:string,end?:boolean){
  const [tabPath,tabQuery]=to.split('?');
  return (!tabQuery||search===`?${tabQuery}`)&&(pathname===tabPath||(!end&&pathname.startsWith(`${tabPath}/`)));
}
...
function PendingTabScreen({to,label,loadingLabel,accountTitle}:{to:string;label:string;loadingLabel:string;accountTitle:string}){
  ...
  return <section className={`${explore?'explore-page':account?'account-page':''} tab-pending-screen${showLoading?' is-loading':''}`} role={showLoading?'status':undefined} aria-label={showLoading?`${loadingLabel} ${label}`:undefined}>
    {showLoading&&(explore?<VStack gap={3} className="tab-pending-discovery">
      ...
      <Skeleton width="56%" height="var(--spacing-9)" radius="rounded"/>
      <figure className="tab-pending-photo"><Skeleton width="100%" height="100%" radius={3}/></figure>
    </VStack>:<>
      <header className="page-intro"><h1>{account?accountTitle:label}</h1></header>
      <VStack gap={4}>
        <HStack gap={3} width="100%"><Skeleton width="var(--spacing-12)" height="var(--spacing-12)" radius="rounded"/><Skeleton width="50%" height="var(--spacing-6)" radius="rounded"/></HStack>
...
export function Shell({children}:{children:React.ReactNode}) {
  ...
  const navigationKey=navigation.location?navigation.location.pathname+navigation.location.search:null;
  const pendingTab=items.find(item=>item.to===navigationKey)?.to;
  const selectedTo=tabIntent?.from===routeKey?tabIntent.to:pendingTab;
  const pendingScreen=selectedTo&&selectedTo!==routeKey?items.find(item=>item.to===selectedTo):null;
  useEffect(()=>{
    if(!tabIntent)return;
    if(tabIntent.from!==routeKey){startedIntent.current=null;setTabIntent(null);return;}
    if(navigation.state!=='idle'){startedIntent.current=tabIntent.to;return;}
    if(startedIntent.current===tabIntent.to){startedIntent.current=null;setTabIntent(null);}
  },[tabIntent,routeKey,navigation.state]);
  const selectTab=(event:React.MouseEvent<HTMLAnchorElement>,to:string)=>{
    if(event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey||event.defaultPrevented||(to===routeKey&&!selectedTo))return;
    startedIntent.current=null;
