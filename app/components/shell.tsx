import { Bell,Search,CalendarDays,MessageCircle,UserRound,ArrowUpRight,ArrowRight,ArrowLeftRight,ChartNoAxesCombined,Check,LogIn } from 'lucide-react';
...
export function Shell({children}:{children:React.ReactNode}) {
  ...
  return <div className={`app-shell ${merchant?'merchant-shell':''}`}>
    ...
    <Modal open={authOpen} onOpenChange={setAuthOpen} title={t('A little closer to your next set','离心仪的美甲更近一步')} description={t('Sign in to book, chat and keep your favourites together.','登录以预约、聊天和保存您喜爱的美甲。')}>
      {enabledProviders.length>0&&<div className="auth-buttons">{enabledProviders.map(provider=><button className="button secondary full" key={provider} disabled={!!busy} onClick={()=>signIn(provider)}><LogIn size={18}/>{t('Continue with','使用')+' '+provider[0].toUpperCase()+provider.slice(1)}</button>)}</div>}
