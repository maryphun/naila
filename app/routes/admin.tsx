...
function MerchantEditor({id,onBack,onChanged}:{id:string;onBack:()=>void;onChanged:()=>void}){
  ...
  return <article className="admin-detail">
    ...
    <section className="admin-editor-section"><h2>Edit merchant information</h2><p className="muted small">Changes apply immediately. Existing booking snapshots are not changed.</p><form className="form-stack" onSubmit={save}>
      ...
      <fieldset className="form-grid"><legend>Business details</legend><label className="field">Area<input name="area" required minLength={2} maxLength={100} defaultValue={merchant.area}/></label><label className="field">Type<select name="type" defaultValue={merchant.type}><option value="home">Home studio</option><option value="studio">Nail studio</option><option value="mobile">Mobile nailist</option></select></label></fieldset>
